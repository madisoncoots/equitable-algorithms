...
# Race blind model calibration plot (reproduction of Figure 3)
race_blind_model <- glm(ground_truth_diabetes ~ ridageyr + bmxbmi,
                        data = data,
                        family = "binomial",
                        weights = round(wtmec8yr/1000))
...
race_blind_calibration_plot_data <- data %>%
  ...
  select(race, risk_score, est_diabetes_prob) %>%
  mutate(risk_score_bin = round(risk_score, 2)) %>% # round to the nearest 0.005
  # mutate(risk_score_bin = as.integer((risk_score * 100) * 2)/2) %>% # round to the nearest 0.005
  # mutate(risk_score_bin = round(risk_score*100/0.5)*0.5/100) %>% # round to the nearest 0.005
  group_by(race, risk_score_bin) %>%
  summarize(diabetes_prev = mean(est_diabetes_prob)) %>%
